...
SELECT DNAME FROM DEPARTAMENTOS ORDER BY LOC ASC;
...
SELECT SAL, ifnull(COMM,0) AS COMM
FROM EMPLEADOS
WHERE DEPTNO = 30
ORDER BY COMM ASC;
...
SELECT DISTINCT ifnull(COMM,"") AS COMM, DEPTNO
FROM EMPLEADOS
ORDER BY COMM;
...
SELECT ENAME
FROM EMPLEADOS
WHERE isnull(COMM) OR COMM <= SAL * 0.25;
...
FROM EMPLEADOS
ORDER BY SAL ASC, SALARIO_TOTAL ASC;
...
SELECT 
	ENAME, 
    IF(isnull(COMM), "NOSESABE", IF(COMM=0, "NO", "SI")) AS COMM, 
    SAL + ifnull(COMM,0) AS SALARIO_TOTAL,
    FLOOR((SAL + ifnull(COMM,0)) * 1.6) AS SALARIO_DEL_PROXIMO_A�O,
    FLOOR(((SAL + ifnull(COMM,0)) * 1.6) * 1.7) AS SALARIO_DEL_SIGUIENTE_A�O
FROM EMPLEADOS
ORDER BY ENAME ASC;
...
SELECT 
    ...
    DEPTNO
FROM EMPLEADOS
WHERE EMPNO REGEXP "7844|7900|7521|7782|7934|7678|7369";
...
SELECT 
    ...
    SAL
FROM EMPLEADOS
WHERE SAL = (SELECT MIN(SAL) FROM EMPLEADOS)
	OR SAL = (SELECT MAX(SAL) FROM EMPLEADOS);
...
SELECT E.ENAME
FROM EMPLEADOS E, DEPARTAMENTOS D
WHERE E.DEPTNO IN (SELECT D2.DEPTNO
				   FROM DEPARTAMENTOS D2
				   WHERE length(D2.LOC) > 5)
...
SELECT DISTINCT DEPTNO, COUNT(*) AS NUM_EMP
FROM EMPLEADOS
GROUP BY DEPTNO
HAVING COUNT(*) > 3;
...
FROM EMPLEADOS E
WHERE E.DEPTNO = 10
	AND E.JOB IN (SELECT E2.JOB 
			     FROM EMPLEADOS E2
                 WHERE E2.DEPTNO = (SELECT D.DEPTNO
...
SELECT ENAME
FROM EMPLEADOS
WHERE EMPNO IN (SELECT MGR FROM EMPLEADOS)
ORDER BY ENAME DESC;
...
SELECT 
    ...
    E.JOB,
    ifnull(E.MGR,"") AS MGR,
    date_format(E.HIREDATE,"%d/%m/%y") AS HIREDATE,
    E.SAL,
    ifnull(E.COMM,"") AS COMM,
    E.DEPTNO
FROM EMPLEADOS E
WHERE E.JOB IN (SELECT E2.JOB
			  FROM EMPLEADOS E2
              WHERE E2.DEPTNO = (SELECT D.DEPTNO
							     FROM DEPARTAMENTOS D
                                 WHERE D.LOC = "CHICAGO"))
...
SELECT D.DNAME
FROM DEPARTAMENTOS D
WHERE D.DEPTNO NOT IN (SELECT DISTINCT E.DEPTNO FROM EMPLEADOS E);
...
SELECT D.DNAME, AVG(E.SAL) * 12, COUNT(*)
FROM EMPLEADOS E, DEPARTAMENTOS D
WHERE E.DEPTNO = D.DEPTNO
GROUP BY D.DNAME;
...
WHERE E.DEPTNO IN (SELECT D.DEPTNO
				   FROM DEPARTAMENTOS D
                   WHERE LOC REGEXP "DALLAS|NEW YORK");
...
FROM EMPLEADOS E
LEFT JOIN EMPLEADOS E2 ON E.MGR = E2.EMPNO;
...
SELECT D.LOC, COUNT(E.EMPNO) AS NUMERO_EMPLEADOS
FROM DEPARTAMENTOS D, EMPLEADOS E
WHERE D.DEPTNO = E.DEPTNO
	AND D.DEPTNO IN (SELECT DISTINCT E2.DEPTNO
					 FROM EMPLEADOS E2)
GROUP BY D.LOC
HAVING NUMERO_EMPLEADOS >= 4;
