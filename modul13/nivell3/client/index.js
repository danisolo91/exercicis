document.addEventListener("DOMContentLoaded", function (event) {
  ...
  const addLegend = document.getElementById('add-legend');
  ...
  function updateEmployeeForm(e) {
    ...
    getEmployee(id).then(data => {
      ...
      addBtn.innerHTML = '<i class="bi bi-pencil-square"></i> Modificar';
    });
  }
  ...
  function restartAddForm() {
    ...
    idField.value = '';
    nameField.value = '';
    addBtn.innerHTML = '<i class="bi bi-plus"></i> Afegir';
  }
    ...
    getEmployees(job).then(employees => {
      ...
      if (employees.length > 0) {
        employees.forEach(employee => {
          ...
          updateBtn.innerHTML = '<i class="bi bi-pencil-square"></i> Modificar';
          updateBtn.dataset.id = employee.id;
          updateBtn.classList = 'btn btn-outline-primary btn-sm mx-1';
          ...
          deleteBtn.classList = 'btn btn-outline-danger btn-sm mx-1';
        ...
        const tr = document.createElement('tr');
        const td = document.createElement('td');
        td.textContent = 'No hi ha cap empleat';
        td.colSpan = 5;
        tr.appendChild(td);
        recordsTable.appendChild(tr);
      }
